...
# Upload SSH public Key To GitHub Secrets
$sshKeyPath = Read-Host "Enter path to your SSH public key (e.g., C:\Users\You\.ssh\temp_vmss_key.pub)"
if (-Not (Test-Path $sshKeyPath)) { throw "SSH public key file not found at $sshKeyPath" }

$sshPublicKey = Get-Content -Raw $sshKeyPath

Write-Host "Uploading SSH public key to GitHub Secrets..."
$secretName = "VMSS_SSH_PUBLIC_KEY"
$tempFile = [System.IO.Path]::GetTempFileName()
$sshPublicKey | Set-Content -Path $tempFile -NoNewline
gh secret set $secretName --repo $Repo --body "$(Get-Content -Raw $tempFile)"
Remove-Item $tempFile
